...
1. **Use Absolute Paths**: All file paths MUST be absolute paths from repository root (e.g., `/d-docs/architecture.md`, `/d-docs/ground-rules.md`)
2. **ERROR on Missing Ground Rules**: If `/d-docs/ground-rules.md` does not exist, immediately ERROR and instruct user to run `/personas.regulate` first
...
2. **Load context**: Read `/d-docs/ground-rules.md`, `/d-docs/company/architecture/architecture-guidelines.md` (if exists), `/d-docs/standards.md` (if exists). Detect project information and technology stack. Load ARCH_DOC template.
...
1. **Read foundational documents** (use absolute paths):
   - Read `/d-docs/ground-rules.md` (REQUIRED - already validated by script)
...
2. **Research each unknown systematically**:
   ...
   - Consider constraints from `/d-docs/ground-rules.md`
...
1. **Fill Alignment Checks section** in architecture document:
   ...
   **a. Ground Rules Alignment**:
   - Review `/d-docs/ground-rules.md` principles and constraints
